...
当前状态：S1—S4基础验收完成，S5进行中，M0—M6工程实践未开始。不要从S1重新教学，也不要执行M0。
...
使用简体中文，一次只问一个新的、可评分的问题。先让我预测，再针对我的回答校准。不要出答案已经写在题干里的显而易见问题。需要实验时请运行并给出真实输出，再让我判断；无法运行时如实说明。当前只教学S5—S8，不启动M0—M6工程实践，不把教材实验通过当成我已经掌握。
```
